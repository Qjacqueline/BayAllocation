...
def calculate_and_merge_inversions(bay_seq, proc_seq):
    ...
    c_i = 0
    for i in range(len(mapped_seq)):
        if c_i <= mapped_seq[i] * 2 + 1:
            for term in distance_ls[c_i:mapped_seq[i] * 2 + 2]:
                if term != "t":
                    while True:
                        if term in resultt:
                            term += "'"
                            ...
                            break
                    resultt.append(term)
            c_i = mapped_seq[i] * 2 + 2
        elif c_i > mapped_seq[i]:
            for term in reversed(distance_ls[mapped_seq[i] * 2 + 1:c_i]):
                if term != "t":
                    while True:
                        if term in resultt:
                            term += "'"
