...
def min_visit_cost(N, A, B):
    dp = [[0] * 2 for _ in range(N + 1)]
    path = [[0] * 2 for _ in range(N + 1)]  # 用于记录路径选择

    # 初始化
    dp[0][0] = abs(B[0]) + abs(B[0] - A[0])  # 访问第 0 对时的代价
    dp[0][1] = abs(A[0]) + abs(A[0] - B[0])  # 访问第 0 对时的代价
...
def find_max_permutation_cost(N, pos):
    ...
    for perm in valid_permutations:
        if perm == [1, 0, 2, 3, 5, 4, 6, 7]:  # 枚举所有排列
            a = 1
        permuted_A = [A[i] for i in perm]
        permuted_B = [B[i] for i in perm]

        # 调用min_visit_cost获取当前排列的代价
        cost, path, dp = min_visit_cost(N, permuted_A, permuted_B)
