...
def CCG(data):
    ...
    while 1:
        ...
        # 预处理子问题
        N, pos = sub_problem_help(data, master_results[0])

        # 求解子问题
        sub_results = sub_problem(N, pos)
...
def master_problem(data, added_cuts):
    ...
    X = [[[] for _ in range(60 * data.K_num)] for _ in range(data.U_num + data.K_num)]
    for j in range(0, 60 * data.K_num):
        for u in range(data.U_num + data.K_num):
    ...
    model.addConstrs((quicksum(X[u][j - 1] for j in data.J) == 1 for u in range(data.U_L_num)), "2b")
    ...
    model.addConstrs((quicksum(X[u][j - 1] for u in range(data.U_num)) <= 2 for j in data.J), "2f")
    # con5: 20和40的不能放在一个贝
    model.addConstrs((X[u][j - 1] + X[uu][j - 1] <= 1 for j in data.J for u in range(0, data.U_L_num)
                      for uu in range(data.U_L_num, data.U_num)), "2g")
    # con6: 放置集装箱数不超过贝位的最大容量
    model.addConstrs((quicksum(data.U_num_set[u] * X[u][j - 1] for u in range(data.U_num)) <= data.S_num * data.T_num
                      for j in range(data.J_num)), "2h")
...
def sub_problem(N, pos):
    ...
    def f_fuc(i, k, ii, jj):
        v = max(min(dp[i][k][j][h] + abs(pos[ii - 1][1 - jj] - pos[i - 1][j]) for j in [0, 1]) for h in [0, 1]) + \
            abs(pos[ii - 1][0] - pos[ii - 1][1]) * cf.inner_unit_p_time
        return v

    def g_fuc(i, k, ii, jj):
        v = min(dp[i][k][j][0] + abs(pos[ii - 1][1 - jj] - pos[i - 1][j]) for j in [0, 1]) \
            + abs(pos[ii - 1][0] - pos[ii - 1][1]) * cf.inner_unit_p_time
        return v
    ...
    # Init state
    ## k=1
    dp[1][1][0][0] = abs(pos[0][1]) + abs(pos[0][1] - pos[0][0]) * cf.inner_unit_p_time  # B_1 -> A_1
    dp[1][1][1][0] = abs(pos[0][0]) + abs(pos[0][0] - pos[0][1]) * cf.inner_unit_p_time  # A_1 -> B_1

    if N > 1:
        dp[2][0][0][0] = abs(pos[1][1]) + abs(pos[1][0] - pos[1][1]) * cf.inner_unit_p_time  # A_2 -> B_2
        dp[2][0][1][0] = abs(pos[1][0]) + abs(pos[1][1] - pos[1][0]) * cf.inner_unit_p_time  # B_2 -> A_2
...
if __name__ == '__main__':
    data = read_data('/Users/jacq/PycharmProjects/BayAllocation/data_process/data/case1')
    CCG(data)
